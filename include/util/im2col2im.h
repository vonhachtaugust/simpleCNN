...
namespace simpleCNN {
     ...
     *
     * Default values are for converting image into col as is i.e. without modification.
     *
     **/
    template<typename T = float_t>
    void im2col_cpu(
        ...
        for (c = 0; c<X_col_num_rows; ++c)
        {
            ...
            for (h = 0; h<height_col; ++h)
            {
                ...
                for (w = 0; w<width_col; ++w)
                {
                    ...
                    X_col.host_at(c, col_index) = im2col_get_pixel(image, image_width, image_height, image_channel,
    ...
    /*
     * Default values are for converting col to image as is, i.e. without modification
     *
     */
    template<typename T = float_t>
    void col2im_cpu(
        ...
        for (c = 0; c<result_num_rows; ++c)
        {
            ...
            for (h = 0; h<height_col; ++h)
            {
                ...
                for (w = 0; w<width_col; ++w)
                {
                    ...
                    col2im_add_pixel(result, image, image_width, image_height, image_row, image_col, image_channel, pad,
                            val);
                }
            }
        }
