  ...
  namespace activation {
    class ReLU : public Activation_layer {
     public:
      ...
      ReLU(const shape4d& shape, const core::backend_t backend_type = core::default_engine()) : Activation_layer(shape, backend_type) {}
      ...
      void forward_activation_gpu(const tensor_t& affine, tensor_t& activated) const override {
        ...
        /** Forward propagate */
        float_t one = 1;
        checkCUDNN(cudnnActivationForward(Activation_layer::cudnnHandle,
                                          Activation_layer::Activation,
                                          &one,
                                          Activation_layer::srcTensorDesc,
                                          affine_gpu,
                                          &one,
                                          Activation_layer::dstTensorDesc,
                                          activated_gpu));
      ...
      void backward_activation_gpu(const tensor_t& affine, const tensor_t& activated, const tensor_t& curr_delta, tensor_t& prev_delta) const override {
        ...
        /** Backward propagate */
        float_t one = 1;
        checkCUDNN(cudnnActivationBackward(Activation_layer::cudnnHandle,
                                           Activation_layer::Activation,
                                           &one,
                                           Activation_layer::dstTensorDesc,
                                           activated_gpu,
                                           Activation_layer::ddstTensorDesc,
                                           curr_delta_gpu,
                                           Activation_layer::srcTensorDesc,
                                           affine_gpu,
                                           &one,
                                           Activation_layer::dsrcTensorDesc,
                                           prev_delta_gpu));
