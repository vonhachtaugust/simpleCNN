  ...
  class ConvCudaForwardOp : public core::OpKernel {
   public:
    ...
    void compute(const core::OpKernelContext& context) override {
      ...
      /** Forward propagate */
      float_t one = 1;
      checkCUDNN(cudnnConvolutionForward(cudnn_handle(), &one, srcTensorDesc, in_data_gpu, weightDesc, weight_gpu,
      ...
      if (params.has_bias) {
        ...
        checkCUDNN(cudnnAddTensor(cudnn_handle(), &one, biasDesc, bias_gpu, &one, dstTensorDesc, out_data_gpu));
        cuda_free(bias_gpu);
      }
   ...
   public:
    ...
    void compute(const core::OpKernelContext& context) override {
      ...
      if (params.has_bias) {
        ...
        checkCUDNN(
          cudnnConvolutionBackwardBias(cudnn_handle(), &one, dstTensorDesc, curr_delta_gpu, &one, dbiasDesc, db_gpu));
