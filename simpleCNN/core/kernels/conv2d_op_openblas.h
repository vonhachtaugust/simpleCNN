...
namespace simpleCNN {
  namespace kernels {
    ...
    inline void conv2d_op_openblas(const tensor_t& previous_layer_input,
      ...
      matrix_t mCurr_delta_no_pad({params.out_channels, params.output_height * params.output_width});
      matrix_t mResult_dW({mPrev_layer_input.rows(), mCurr_delta_no_pad.rows()});
      ...
      for (size_t i = 0; i < params.batch_size; ++i) {
        ...
        multiply_2_dim_tensors_float(mWeights, mCurr_delta, mResult_delta, false, false);
        ...
        im2col_cpu(previous_layer_input, i, mPrev_layer_input, params.in_channels, params.input_height,
                   params.input_width, params.filter_size(), params.stride(), params.padding);
        im2col_cpu(curr_delta, i, mCurr_delta_no_pad, params.out_channels, params.output_height, params.output_width);

        multiply_2_dim_tensors_float(mPrev_layer_input, mCurr_delta_no_pad, mResult_dW, false, true);
