...
namespace simpleCNN {
  ...
  template <typename T>
  void print_pack(std::ostream& out,
                  T t) {  // TODO: C++17 allows easier printing
    out << t;
  }
  ...
  inline std::ostream& operator<<(std::ostream& os, const tensor_t& tensor) {
    for (int i = 0; i < tensor.depth(); i++) {
      print_matrix(os, i, tensor);
    }
  }
  ...
  /**
     ...
     *
     **/  //--------------------------------------------------------//
  void matrix_multiplcation(float* A,
                            int A_width,
                            int A_height,
                            float* B,
                            int B_width,
...
#define TRANSPOSE(X) ((X) ? CblasTrans : CblasNoTrans)
    ...
    cblas_sgemm(CblasRowMajor,  // I know you
                TRANSPOSE(tA), TRANSPOSE(tB), m, n, k, 1.0f, A, lda, B, ldb,
                beta, AB, n);

#undef TRANSPOSE(X)
  }

  void multiply_2_dim_tensors_float(matrix_t& A,
                                    matrix_t& B,
                                    matrix_t& AB,
                                    bool transpose_A,
                                    bool transpose_B) {
    matrix_multiplcation(&A.host_at(0, 0), A.cols(), A.rows(), &B.host_at(0, 0),
                         B.cols(), B.rows(), &AB.host_at(0, 0),  // A * B
